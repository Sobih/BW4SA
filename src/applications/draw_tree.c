 ...
 * @bug		No known bugs.
 */
internal_node* find_node_by_substring(substring* substr, internal_node* root) {
...
void collect_internal_nodes(iterator_state* state, void* results) {
	...
	tree_results* result = (tree_results*) results;
	...
	insert_node->id = result->node_index;
	result->node_index++;
	internal_node* prev_node = find_node_by_substring(state->prev, result->root);
	insert_node->c = state->current_extension;

	insert_node->weiner_node = prev_node;
	internal_node* temp_node = result->root->first_child;

	//if this node is first to be put in the tree, it will be the first child of the root node
	if (temp_node == NULL) {
		result->root->first_child = insert_node;
		insert_node->parent = result->root;
		insert_node->weiner_node = result->root;
		return;
	}
 ...
 * @bug		No known bugs.
 */
void print_tree_to_file(char* filename, int* suffix_array, char* orig_string, internal_node* root) {
//	printf("starting printin'\n");
	FILE* f = fopen(filename, "w");
	fprintf(f, "//This is a suffix tree generated by the BW4SA library\n");
...
parameter_struct* initialize_for_tree_drawing(char** strings) {
	tree_results* results = malloc(sizeof(tree_results));
	results->root = calloc(1, sizeof(internal_node));
	results->root->id = 0;
	results->root->substr = calloc(1, sizeof(substring));
	results->root->substr->normal = ((interval ) { .i = 0, .j = strlen(strings[0]) } );
	results->root->c = ' ';
	results->node_index = 1;

	parameter_struct* params = malloc(sizeof(parameter_struct));
	params->callback = &collect_internal_nodes;
	params->iterate_type = DOT_TREE;
...
void draw_suffix_tree(char* string, char* filename) {
	...
	strings[0] = string;
	parameter_struct* params = initialize_for_tree_drawing(strings);
	iterate(params);
	tree_results* results = (tree_results*) params->ret_data;
	int length = strlen(filename);
	char* path = malloc(sizeof(char) * (length + 4));
	strcpy(path, filename);
	path[length] = '.';
	path[length + 1] = 'g';
	path[length + 2] = 'v';
	path[length + 3] = 0;
	print_tree_to_file(path, suffix_array, string, results->root);
