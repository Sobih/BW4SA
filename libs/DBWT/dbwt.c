...
// T[n-1] �͕K��TYPE_L
  ...
  for (i=n-2; i>=0; i--) {
    ...
    if (gc(i) < gc(i+1)) {
      ...
      if (tt == TYPE_S) {
        p = i+1;
        pa_set(T2,j--,getname(h1,q-p+1,&T[p])); // T[p..q] ��name�� T2 �ɏ���
        q = p;
      }
      t = TYPE_L;
    }
  ...
  pa_set(T2,0,s1); // T2[0] = s1 (T�̐擪�̕������name)
...
////////////////////////////////////
  ...
  for (i=0; i<n1; i++) {
    ...
    uchar *q;
    p = sa[i];
    q = S[pa_get(T2,p-1)]; // bw[i] = T2[sa[i]-1] �̕�����
    l = getlen(&q);
    q += 1; // for sentinel
  ...
  for (c = SIGMA; c>=0; c--) {
    ...
    for (t = 1; t >= 0; t--) { // TYPE_L, TYPE_S �̏��ɏ���
      while (!emptyqueue(Q[t][c])) {
        ...
        c1 = q[-1];  // c2 = q[0]; // c2+1 = c
        if (c1 <= (c-1)) { // TYPE_S
          enqueue(Q[TYPE_L][c1+1],(q-1) - bwp_base); // ����̂͏�� L
          if (q-1 == lastptr) {
            last = M2[c1+1]--;
          } else {
            c0 = q[-2];
